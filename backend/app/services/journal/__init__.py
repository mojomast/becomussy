...
class JournalService:
    ...
    async def search(
        ...
        if params.entry_type:
            ...
            count_query = count_query.where(
                JournalEntry.entry_type == params.entry_type
            )
        ...
        if params.date_from:
            ...
            count_query = count_query.where(
                JournalEntry.timestamp >= params.date_from
            )
        if params.date_to:
            query = query.where(JournalEntry.timestamp <= params.date_to)
            count_query = count_query.where(
                JournalEntry.timestamp <= params.date_to
            )

        # Filter by linked project (uses PostgreSQL array @> operator)
        if params.linked_project_id:
            project_filter = JournalEntry.linked_project_ids.contains(
                [params.linked_project_id]
            )
            query = query.where(project_filter)
            count_query = count_query.where(project_filter)

        # Filter by linked identity theme (uses PostgreSQL array @> operator)
        if params.linked_theme:
            theme_filter = JournalEntry.linked_identity_themes.contains(
                [params.linked_theme]
            )
        ...
        query = (
            query.order_by(JournalEntry.timestamp.desc())
            .limit(params.limit)
    ...
    async def update(
        ...
        # Map the 'provenance' field name to the column name
    ...
    async def summarize(
        ...
        For MVP this returns the raw entries. Actual AI summarization
        is post-MVP and will be layered on top.
        """
        query = (
            select(JournalEntry)
